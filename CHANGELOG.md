...
**问题**：末墩第 4 张牌打出的瞬间 RoundEnd 状态即被应用——底牌揭示与结果面板立刻弹出，整墩牌还没看清就被结算界面盖住；而前几家 AI 出牌之间都有 ~800ms 节奏，唯独最后一墩没有停顿。

**修复**：`submitPlay` 与 `runAiTurns`（AI 正常/fallback 两分支）在结果为 RoundEnd 时不再立即应用终态——先以 Playing 相位应用中间态让出牌落桌（settledTrick 四张 + 👑 展示、底牌/结果面板不出现），停顿 tick(800)（与前几家 AI 出牌间隔一致）后再应用真正终态（底牌揭示 + 结果面板 + 本局结束文案），4 秒后照旧开新局。debug 的 aiReasons 附在中间态上，延迟应用时并入终态避免丢失。

E2E 实测（人类/AI 两路径）：停顿期仍 Playing、无面板、四张已落桌且手牌已空；~800ms 后 RoundEnd 面板与文案出现；aiReasons 保留。
...
### 领出只剩一张：自动选上并锁定，不自动打出

**问题**：领出阶段手牌只剩最后一张时，玩家还得手点这张唯一的牌再点出牌——与跟牌"唯一可出自动锁定"的体验不一致；但剩一对时仍需玩家自行决定拆单张还是成对领出，不应代劳。

**修复**：GameTable 新增 effect——轮到自己**领出**（trickPlays 为空）且手牌恰剩 1 张 → 自动选中并锁定（不可放下），出牌仍由玩家手动确认，不自动提交；剩一对（hand.length ≠ 1）不触发。与末墩自动打分支互斥（该分支仅跟牌 trickPlays 非空时生效）。

E2E 实测：末张领出自动选中+锁定、不自动打出；剩一对不代选；多张领出不代选。
...
**问题**：`.score-points` 设有 `max-height: 64px; overflow-y: auto`——每排约 20px，即最多显示 3 排（15 张）分牌，超出的部分被压进一个要滚动的小窗口里，对局中几乎不可发现还有分牌没看到。

**修复**：移除 `max-height` 与 `overflow-y`，分牌网格随数量自然向下生长（score-display 为 absolute 定位，生长只覆盖桌布空区，不挤压其他组件）。实测注入 24 张分牌（两副牌全部计分牌，8×10 + 8×K + 8×5）：24/24 全部渲染、5 排完整可见、无滚动无截断。

无新增测试。

- **影响文件**：`packages/client/src/components/game/GameTable.css`
...
**问题**：第四家吊主单张、能盖过对手时，无 A 可盖的牌一律降序取最大——如手牌 ♠Q♠Q + ♠J、领出 ♠10，拆 ♠Q 对出 ♠Q，而 ♠J 单张（11 > 10）就能赢墩；「用最小牌盖」文案与实际行为不符。

**修复**：第四家能盖过对手（墩上有分/无分统一）：① 最小能盖过是单张 → 出最小单张（同等级单张优先于对子，不拆对）；② 最小能盖过是对子 X → 按「加分不拆对的垫牌优先级」（position-policy 的 add 模式类别）判断：X 类别 < 更大单张类别 → 拆 X 出一张盖过；否则出更大单张保留对子；③ 对子/拖拉机领出：优先选含最多分且能盖过的（10 对 > K/5 对），同分取最小能盖过；尽量不拆拖拉机（拖拉机外对子优先，没得选才拆）。实测：原场景（♠Q♠Q ♠J 盖 ♠10）现在建议 ♠J；引擎全量 723 项通过。

**新增 10 项测试**（ai-position-follow.test.ts：10 项）、**修改 1 项测试**（arena-e2e.test.ts：镜像对局走向变化，oppLevel 快照 229 → 210），引擎 723 项 + arena 65 项 + CLI 80 项 + client 95 项 = 963 项通过。

- **影响文件**：`packages/engine/src/ai/follow-trump.ts`、`packages/engine/src/ai/position-policy.ts`、`packages/engine/src/__tests__/ai-position-follow.test.ts`
